...
type Section = "Dashboard" | "System Health" | "Monitoring" | "Storage" | "Processes" | "Network" | "Action Audit";
...
export default function App() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav>
        {(["Dashboard", "System Health", "Monitoring", "Storage", "Processes", "Network", "Action Audit"] as Section[]).map((item) => <button key={item} className={`nav-item ${section === item ? "active" : ""}`} onClick={() => setSection(item)}>{item}</button>)}
    ...
    <section className="content">
      <header className="header"><div><p className="eyebrow">{section.toUpperCase()}</p><h1>{section === "Dashboard" ? "Your Linux system, at a glance." : section}</h1><p className="subtitle">{section === "Monitoring" ? "Live read-only metrics with a bounded in-memory history." : section === "Action Audit" ? "Local execution history for explicitly confirmed safe actions." : "Read-only insights, collected through Linux-native capabilities."}</p></div><button className="primary" onClick={() => void refresh()} disabled={loading}>{loading ? "Refreshing…" : "Refresh"}</button></header>
