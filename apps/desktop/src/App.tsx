...
import { AlertEventHistory } from "./components/AlertEventHistory";
...
type Section = "Dashboard" | "System Health" | "Health Workspace" | "Alert History" | "Monitoring" | "Storage" | "Processes" | "Network" | "Action Audit";
...
export default function App() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav>
        {(["Dashboard", "System Health", "Health Workspace", "Alert History", "Monitoring", "Storage", "Processes", "Network", "Action Audit"] as Section[]).map((item) => <button key={item} className={`nav-item ${section === item ? "active" : ""}`} onClick={() => setSection(item)}>{item}</button>)}
    ...
    <section className="content">
      <header className="header"><div><p className="eyebrow">{section.toUpperCase()}</p><h1>{section === "Dashboard" ? "Your Linux system, at a glance." : section}</h1><p className="subtitle">{section === "Monitoring" ? "Live read-only metrics with a bounded in-memory history." : section === "Alert History" ? "A local, auditable record of alert decisions." : section === "Action Audit" ? "Local execution history for explicitly confirmed safe actions." : section === "Health Workspace" ? "A focused, explicit path from health signals to safe diagnostics." : "Read-only insights, collected through Linux-native capabilities."}</p></div><button className="primary" onClick={() => void refresh()} disabled={loading}>{loading ? "Refreshing…" : "Refresh"}</button></header>
      ...
      {section === "Alert History" && <AlertEventHistory />}
