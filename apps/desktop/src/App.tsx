...
function Sparkline({ values }: { values: number[] }) {
  ...
  return <svg className="sparkline" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none"><polyline points={points} fill="none" stroke="currentColor" strokeWidth="2.5" vectorEffect="non-scaling-stroke" /></svg>;
}
...
export default function App() {
  ...
  async function refresh() {
    ...
    try {
      const [nextSystem, nextStorage, nextProcesses, nextNetwork] = await Promise.all([
        invoke<SystemStatus>("system_status"), invoke<FilesystemStatus[]>("storage_status"),
        invoke<ProcessInfo[]>("process_status"), invoke<NetworkInterface[]>("network_status"),
      ]);
  ...
  async function sampleMonitoring() {
    try {
      const snapshot = await invoke<MonitorSnapshot>("monitor_snapshot");
      setHistory((current) => [...current.slice(-119), snapshot]);
    } catch (err) { setError(String(err)); }
  }

  useEffect(() => { void refresh(); void sampleMonitoring(); }, []);
  useEffect(() => {
    const timer = window.setInterval(() => { void sampleMonitoring(); }, 1000);
    ...
    <main className="app-shell">
      ...
      <section className="content">
        <header className="header">
          <div><p className="eyebrow">{section.toUpperCase()}</p><h1>{section === "Dashboard" ? "Your Linux system, at a glance." : section}</h1><p className="subtitle">{section === "Monitoring" ? "Live read-only metrics with a bounded in-memory history." : "Read-only insights, collected through Linux-native capabilities."}</p></div>
        ...
        {section === "Monitoring" && <section className="monitoring">
          <div className="monitor-grid">
            ...
            <article className="monitor-card"><div className="monitor-heading"><span className="label">MEMORY</span><strong>{latest ? `${latest.memory_percent.toFixed(1)}%` : "—"}</strong></div><Sparkline values={history.map((item) => item.memory_percent)} /><span className="muted">Current memory utilization</span></article>
            <article className="monitor-card"><div className="monitor-heading"><span className="label">SWAP</span><strong>{latest ? `${latest.swap_percent.toFixed(1)}%` : "—"}</strong></div><Sparkline values={history.map((item) => item.swap_percent)} /><span className="muted">Current swap utilization</span></article>
          </div>
          <article className="card"><span className="label">NETWORK THROUGHPUT</span>{latest?.network.map((item) => <div className="rate-row" key={item.name}><strong>{item.name}</strong><span>↓ {formatRate(item.rx_bytes_per_second)} · ↑ {formatRate(item.tx_bytes_per_second)}</span></div>)}</article>
          <p className="monitor-note">Live history keeps the latest 120 samples in memory and is discarded when Linux Powerhouse exits.</p>
        </section>}

        {section === "Dashboard" && system && <section className="grid">
