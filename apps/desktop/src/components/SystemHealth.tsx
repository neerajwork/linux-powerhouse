...
function loadHistory(): HistoryPoint[] {
  try {
    ...
    return parsed.slice(-MAX_HISTORY).filter((point): point is HistoryPoint => {
      ...
      return typeof item.timestamp === "number" && typeof item.total_anomalies === "number" &&
        typeof item.storage_anomalies === "number" && typeof item.process_anomalies === "number" &&
        typeof item.network_anomalies === "number" && typeof item.service_anomalies === "number" &&
        (item.health === "Healthy" || item.health === "Attention" || item.health === "Degraded");
    });
  } catch {
    return [];
  }
...
function saveHistory(history: HistoryPoint[]) {
  try {
    ...
    // History persistence is best-effort and must never prevent health inspection.
  }
}
...
export function SystemHealth() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const point = { ...result, timestamp: Date.now() };
      setSnapshot(result);
      setHistory((current) => {
        const next = [...current, point].slice(-MAX_HISTORY);
        saveHistory(next);
        return next;
      });
    } catch (err) {
