...
const reasonLabel = (reason: AlertEvent["reason"]): string => {
  switch (reason) { case "ActivePolicy": return "Active policy"; case "Snoozed": return "Snoozed"; case "Dismissed": return "Dismissed"; case "SnoozeExpired": return "Snooze expired"; case "CriticalOverride": return "Critical override"; }
};
const explanation = (event: AlertEvent): { headline: string; detail: string; action: string } => {
  ...
  let action: string;
  switch (event.reason) { case "CriticalOverride": action = "Critical events remain visible regardless of routine warning preferences."; break; case "Snoozed": action = "The routine warning was snoozed; review the event if the underlying condition persists."; break; case "Dismissed": action = "The routine warning was dismissed; review the history if the condition returns."; break; case "SnoozeExpired": action = "The snooze period had expired, so the routine warning returned to the active policy."; break; case "ActivePolicy": action = "The active alert policy determined how this event was handled."; break; }
...
const primaryEvidence = (event: AlertEvent, snapshot: MonitorSnapshot): string => {
  switch (event.kind) { case "Cpu": return `CPU utilization was ${snapshot.cpu_percent.toFixed(1)}% near the alert.`; case "Memory": return `Memory utilization was ${snapshot.memory_percent.toFixed(1)}% near the alert.`; case "Swap": return `Swap utilization was ${snapshot.swap_percent.toFixed(1)}% near the alert.`; case "Storage": return `Storage I/O was ${snapshot.storage_read_bytes_per_second.toFixed(1)} read / ${snapshot.storage_write_bytes_per_second.toFixed(1)} write bytes/s near the alert.`; case "Network": return `Network activity was sampled near the alert; ${snapshot.process_count} processes were present at that time.`; }
};
...
export function AlertEventHistory() {
  ...
  const load = useCallback(async () => {
    try { setError(null); const [history, monitorHistory] = await Promise.all([invoke<AlertEvent[]>("alert_event_history"), invoke<MonitorSnapshot[]>("monitor_history")]); setEvents(history); setSnapshots(monitorHistory); }
  ...
  async function clearHistory() { try { setError(null); await invoke("clear_alert_event_history"); setEvents([]); } catch (err) { setError(String(err)); } }
  ...
  return (
    <section className="alert-history" aria-labelledby="alert-history-title">
      <div className="alert-history__header"><div><p className="eyebrow">ALERT HISTORY</p><h2 id="alert-history-title">Alert event history</h2><p className="subtitle">Review warning and critical decisions with focused local filters, deterministic explanations, performance evidence, and process contributors.</p></div><button className="secondary" onClick={() => void clearHistory()} disabled={!events.length}>Clear history</button></div>
      <AlertHistorySummary events={events} /><AlertHistoryInsights events={events} />
      ...
      {!!filteredEvents.length && <div className="list alert-history__list">{[...filteredEvents].reverse().map((event, index) => {
        const why = explanation(event); const snapshot = nearestSnapshot(event, snapshots); const evidence = snapshot ? primaryEvidence(event, snapshot) : "No matching performance snapshot was available for this alert."; const evidenceAge = snapshot && event.performance_timestamp_ms != null ? Math.abs(snapshot.timestamp_ms - event.performance_timestamp_ms) : null; const processEvidence = event.process_evidence ?? [];
        return <article className="row" key={`${event.timestamp_ms}-${event.kind}-${index}`}><div><strong>{categoryLabel(event.kind)} · {event.severity}</strong><span>{new Date(event.timestamp_ms).toLocaleString()} · {event.value.toFixed(1)}%</span><small>{reasonLabel(event.reason)}</small><details><summary>Why did this alert happen?</summary><div className="alert-history__explanation"><strong>{why.headline}</strong><span>{why.detail}</span><small>{why.action}</small><div className="alert-history__evidence"><strong>Performance evidence</strong><span>{evidence}</span>{snapshot && evidenceAge != null && <small>Snapshot {evidenceAge} ms from the alert context · memory {snapshot.memory_percent.toFixed(1)}% · swap {snapshot.swap_percent.toFixed(1)}% · {snapshot.process_count} processes · {snapshot.running_processes} running</small>}</div>{(event.kind === "Cpu" || event.kind === "Memory") && <div className="alert-history__evidence"><strong>Process contributors at alert time</strong>{processEvidence.length ? processEvidence.map((process) => <span key={`${process.pid}-${process.rank}`}>#{process.rank} {process.name} · PID {process.pid} · {processMetric(event, process)}</span>) : <span>No process evidence was available for this event.</span>}<small>These are bounded local observations, not proof that a process caused the alert.</small></div>}</div></details></div><b>{event.decision === "Notify" ? "Notified" : "Suppressed"}</b></article>;
      })}</div>}
